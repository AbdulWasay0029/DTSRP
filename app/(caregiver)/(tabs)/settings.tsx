...
export default function CaregiverSettingsScreen() {
    ...
    const { patients } = useMedicineStore();
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
                ...
                {/* Connected Patients Group */}
                <View style={styles.section}>
                    <Text style={styles.sectionTitle}>CONNECTED PATIENTS</Text>
                    <View style={styles.settingsGroup}>
                        {patients.map((p) => (
                            <View key={p.id} style={styles.settingItem}>
                                <View style={styles.settingLeft}>
                                    <Image source={{ uri: `https://ui-avatars.com/api/?name=${p.name}&background=0f172a&color=fff` }} style={styles.patientAvatar} />
                                    <View>
                                        <Text style={styles.settingLabel}>{p.name}</Text>
                                        <Text style={styles.patientId}>ID: #{p.inviteCode || p.id.substring(0, 4)}</Text>
                                    </View>
                                </View>
                            </View>
                        ))}

                        <TouchableOpacity style={[styles.settingItem, styles.settingItemLast, styles.addPatientBtn, patients.length === 0 && { borderBottomWidth: 0 }]} onPress={() => router.push('/(caregiver)/addPatient')} activeOpacity={0.7}>
...
const styles = StyleSheet.create({
    ...
    patientAvatar: { width: 36, height: 36, borderRadius: 18 },
    patientId: { fontSize: 12, color: '#64748b', marginTop: 2 },
    addPatientBtn: { justifyContent: 'center' },
    addPatientText: { fontSize: 14, fontWeight: '600', color: '#19e66f', textAlign: 'center', width: '100%' },
